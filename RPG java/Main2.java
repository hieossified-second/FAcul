...
public class Main2 {
    ...
    public static Player criarPlayerAdmin() {
        ...
        while (true) {
            ...
            System.out.println(cores.negrito + cores.vermelhoForte + "1) GUERREIRO" + cores.reset);
            ...
            switch (escolha) {
                ...
                default:
                    ...
                    return Player.criarPlayer(rpgClasses.GUERREIRO); // Define automaticamente como GUERREIRO
        ...
        player.vida = sc.nextInt();

        player.vidaMax = player.vida;

        System.out.println("Dano: ");
        player.dano = sc.nextInt();

        System.out.println("Cura: ");
        player.cura = sc.nextInt();
        player.quantidadeCura = player.cura;
        limparTela();
    ...
    public static Inimigo escolherinimigo(){
        ...
        while (true) {
            ...
            System.out.println(cores.vermelho +"5) REI DEMONIO" + cores.reset);
            System.out.println(cores.vermelhoForte + "6) ???"+ cores.reset);
            ...
            switch (escolha) {
                ...
                case 1:
                    return Inimigo.gerarInimigo(Inimigos.ESQUELETO);

                case 2:
                    return Inimigo.gerarInimigo(Inimigos.GOBLIN);

                case 3:
                    return Inimigo.gerarInimigo(Inimigos.DEMONIO);

                case 4:
                    return Inimigo.gerarInimigo(Inimigos.CAVALEIRONEGRO);

                case 5:
                    return Inimigo.gerarInimigo(Inimigos.REIDemonio);

                case 6:
                    return Inimigo.gerarInimigo(Inimigos.DEVORADORdeUNIVERSOS);

                default:
                    System.out.println("Apenas escolha uma das opcoes!");
                    limparTela();
            }
            limparTela();
        }
    ...
    public static class Inimigos {
        ...
        public static final String DEVORADORdeUNIVERSOS = "DEVORADOR DE UNIVERSOS";
    }
    ...
    public static class Inimigo {
        ...
        public static Inimigo gerarInimigo(String nomeInimigo) {
            ...
            if (nomeInimigo.equals(Inimigos.ESQUELETO)) {
                ...
                Inimigo i = new Inimigo();
                i.vida = 220;
                i.dano = rd.nextInt(40) + 12;
                i.nome = Inimigos.REIDemonio;
                return i;
            } else if (nomeInimigo.equals(Inimigos.DEVORADORdeUNIVERSOS)){
                Inimigo i = new Inimigo();
                i.vida = 4000;
                i.dano = 80;
                i.nome = Inimigos.DEVORADORdeUNIVERSOS;
                return i;
            }
    ...
    public static int chanceAtaque(){
        Random rd = new Random();
        return rd.nextInt(2);
    }


    public static boolean combate(Player player, Inimigo enemy, Scanner sc) throws InterruptedException {
        ...
        digitar("\nApareceu um " + cores.vermelho + enemy.nome + cores.reset + "!!");
        ...
        while (player.vida > 0 && enemy.vida > 0) {
            ...
            if (!sc.hasNextInt()) {
                limparTela();
                digitar(cores.vermelho + "❌Escolha Inválida. Digite um número de 1 a 3." + cores.reset);
                sc.next();
                Thread.sleep(1000);
                limparTela();
                continue;
            ...
            switch (escolha) {
                ...
                case 1:
                    ...
                    if (chanceAtaque() == 1) {
                        ...
                        digitar(" ");
                        digitar(cores.azul + "Voce inferiu: " + player.dano + cores.reset + cores.vermelho + " de dano" + cores.reset);
                        ...
                        if (enemy.vida > 0) {
                            ...
                            if (chanceAtaque() == 1){
                                ...
                                digitar(cores.vermelho + "💥 O " + enemy.nome + " contra-atacou!" + cores.reset);
                                digitar("🩸Dano recebido: " + cores.vermelho + " -" + enemy.dano + cores.reset);
                            }
                            else {
                                tomouDano = false;

                                digitar(cores.roxo + "Inimigo Errou o Ataque ❌" + cores.reset);
                                digitar(cores.azul + "Sua vida atual: " + player.vida + cores.reset);
                            }
                        }
                    }
                    else{
                        digitar("Voce errou o Ataque ❌");
                        ...
                        if (chanceAtaque() == 1){
                            ...
                            tomouDano = false;
                            digitar(" ");
                            digitar("Inimigo tambem Errou o Ataque ❌");
                        }
                    }
        ...
        if (player.vida <= 0) {
            ...
            digitar("=======🎖️Você venceu " + cores.vermelho + enemy.nome + cores.reset + "======" );
            Thread.sleep(2500);
            return true;
        }
    }
    ...
    public static void escolherEAplicarBuff(Player player, Scanner sc, int nivelBuff) throws InterruptedException {
        ...
        limparTela();
        digitar(cores.verde + "Parabéns! Você derrotou o inimigo e ganhou um buff de Nível " + nivelBuff + "!" + cores.reset);
        Thread.sleep(1500);
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        if (combate(player , inimigo_Goblin, sc)) {
            player.ResetVida();
            escolherEAplicarBuff(player, sc, 2);
        } else {
            return;
        }
        if (combate(player , inimigo_Demonio, sc)) {
            player.ResetVida();
        ...
        if (combate(player , inimigo_CavaleiroNegro, sc)) {
            player.ResetVida();
            escolherEAplicarBuff(player, sc, 4);
        } else {
            return;
        }
        if (combate(player , inimigo_ReiDemonio, sc)) {
            player.ResetVida();
